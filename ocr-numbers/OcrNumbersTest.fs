...
[<Fact>]
let ``symbolToNumber Test`` () =
    let three = [ " _ "; " _|"; " _|"; "   " ]
    let four = [ "   "; "|_|"; "  |"; "   " ]
    let five = [ " _ "; "|_ "; " _|"; "   " ]
    symbolToNumber three |> should equal (Some "3")
    symbolToNumber four |> should equal (Some "4")
    symbolToNumber five |> should equal (Some "5")

[<Fact>]
let ``symbolsToNumberTest`` () =
    let one = [ "   "; "  |"; "  |"; "   " ]
    let zero = [ " _ "; "| |"; "|_|"; "   " ]
    symbolsToNumber [one; one; zero] |> should equal ("110")

[<Fact>]
let ``lineToSymbolTest`` () =
    let one = [ "   "; "  |"; "  |"; "   " ]
    let zero = [ " _ "; "| |"; "|_|"; "   " ]
    let rows =
        [ "       _ ";
          ...
          "  |  ||_|";
          "         "; ]
    lineToSymbol rows |> should equal ([one; one; zero])
